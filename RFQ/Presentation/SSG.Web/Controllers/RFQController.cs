...
namespace SSG.Web.Controllers
{
    public partial class RFQController : BaseSSGController
    {
        ...
        private readonly IRFQBuyerHistoryService _rfqBuyerHistoryService;
        ...
        public RFQController(IRFQService rfqService,
            ...
            IRFQBuyerHistoryService rfqBuyerHistoryService,
            ILogger logger
            )
        {
            ...
            this._rfqBuyerHistoryService = rfqBuyerHistoryService;
            ...
            if (rfq != null)
            {
                if (rfq.UserCanSubmit(_workContext.CurrentUser))
                {
                    try
                    {
                        ...
                        rfq.EditUrl = Url.Action("RemoteLogin", "User", new { userId = _workContext.CurrentUser.Id, returnUrl = Url.Action("Edit", "RFQ", new { id = rfq.Id }, this.Request.Url.Scheme) }, this.Request.Url.Scheme);

                        _rfqService.SaveRFQ(rfq);

                        //Insert buyer history...
                        _rfqBuyerHistoryService.SaveBuyerHistory(rfq.Id, rfq.BuyerId, rfq.BuyerId, "Buyer history create on submit.", _workContext.CurrentUser.Id);
                    }
                    catch (Exception ex)
                    {
        ...
        public virtual ActionResult ReassignBuyer(ReassignBuyerModel model)
        {
            if (ModelState.IsValid)
            {
                var rfq = _rfqService.GetById(model.RFQId);

                if (rfq != null)
                {
                    rfq.BuyerId = model.BuyerId;
                    rfq.BuyerChangeReason = model.ChangeReason;
                    rfq.PopulateAuditFieldsForEdit(_workContext.CurrentUser.Id);
                    rfq.SetEmailAttachmentFilePath(Server.MapPath("~/Email/RFQ/"));
                    rfq.EditUrl = Url.Action("RemoteLogin", "User", new { userId = _workContext.CurrentUser.Id, returnUrl = Url.Action("Edit", "RFQ", new { id = rfq.Id }, this.Request.Url.Scheme) }, this.Request.Url.Scheme);
            ...
            model.PopulateLookupsForReassignBuyer(_userService);
            return PartialView("_ReassignBuyer", model);
        }

        public virtual ActionResult ReassignBuyer(int rfqId)
        {
            if (_workContext.CurrentUser.IsBuyer(true))
            {
                var rfq = _rfqService.GetById(rfqId);
                var model = new ReassignBuyerModel();

                if (rfq != null && rfq.RFQStatusId == (int)RFQStates.Submitted)
                {
                    model = Mapper.Map<ReassignBuyerModel>(rfq);
                    model.PopulateLookupsForReassignBuyer(_userService);
                    
                    return PartialView("_ReassignBuyer", model);
                }
                else
                    return AccessDeniedPage("Cannot reassign buyer because RFQ must be in submitted state!");
            }
            else
                return AccessDeniedPage("You must be a buyer to reassign RFQ Buyer!");
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            if (rfq != null)
            {
                ...
                if (!rfq.UserCanEdit(_workContext.CurrentUser))
                    return AccessDeniedPage("You must be the specified requestor or buyer inorder to edit an RFQ. If you are the requestor then the RFQ must be submitted.");
        ...
        public virtual ActionResult CreateCopy(int rfqId)
        {
            ...
            if (existingRFQ != null)
            {
                ...
                try
                {
                    ...
                    // Do not include deleted lines because it will only take up space...
                    newRFQ.Lines = newRFQ.Lines.Where(x => x.Active == true).ToList();
                    ...
                    foreach (var line in newRFQ.Lines)
                    {
                        ...
                        line.Attachments = line.Attachments.Where(q => q.Active == true).ToList();
                        ...
                        line.FirstUploadDate = null;

                        line.Quotations = new List<Quotation>();
                    }
        ...
        public virtual ActionResult ViewRFQ(int id)
        {
            var query = _rfqService.GetById(id);

            var model = new RFQModel();
            model.Id = query.Id;
            model.RFQNo = query.RFQNo;
            model.Department = query.Department;
            model.Buyer = query.Buyer.GetFullName();
            model.Requester = query.Requestor.GetFullName();
            model.Subject = query.Subject;
            model.DateCreatedOnUtc = query.DateCreatedOnUtc;
            model.Status = query.RFQStatus.Name;


            return View(MVC.RFQ.Views.ViewRFQ,model);

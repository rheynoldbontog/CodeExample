...
{
    [AdminAuthorize]
    public partial class CurrencyController :  BaseSSGController
    {
        #region Fields
        ...
        public ActionResult List(bool liveRates = false)
        {
            ...
            foreach (var currency in currenciesModel)
                currency.IsPrimarySiteCurrency = currency.Id == _currencySettings.PrimarySiteCurrencyId ? true : false;
            if (liveRates)
            {
                try
                {
                    ...
                    if (primaryExchangeCurrency == null)
                        throw new SSGException("Primary exchange rate currency is not set");
        ...
        public ActionResult MarkAsPrimarySiteCurrency(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCurrencies))
                return AccessDeniedView();

            _currencySettings.PrimarySiteCurrencyId = id;
            _settingService.SaveSetting(_currencySettings);
            return RedirectToAction("List");
        }
        ...
        public ActionResult Delete(int id)
        {
            ...
            try
            {
                if (currency.Id == _currencySettings.PrimarySiteCurrencyId)
                    throw new SSGException(_localizationService.GetResource("Admin.Configuration.Currencies.CantDeletePrimary"));

                if (currency.Id == _currencySettings.PrimaryExchangeRateCurrencyId)
                    throw new SSGException(_localizationService.GetResource("Admin.Configuration.Currencies.CantDeleteExchange"));

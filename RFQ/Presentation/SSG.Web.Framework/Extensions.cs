...
namespace SSG.Web.Framework
{
    public static class Extensions
    {
        ...
        public static GridBoundColumnBuilder<T> Righted<T>(this GridBoundColumnBuilder<T> columnBuilder) where T : class
        {
            return columnBuilder.HtmlAttributes(new { align = "right" })
                            .HeaderHtmlAttributes(new { style = "text-align:center;" });
        }

        public static GridTemplateColumnBuilder<T> Righted<T>(this GridTemplateColumnBuilder<T> columnBuilder) where T : class
        {
            return columnBuilder.HtmlAttributes(new { align = "right" })
                            .HeaderHtmlAttributes(new { style = "text-align:center;" });
        }

        public static GridBoundColumnBuilder<T> Lefted<T>(this GridBoundColumnBuilder<T> columnBuilder) where T : class
        {
            return columnBuilder.HtmlAttributes(new { align = "left" })
                            .HeaderHtmlAttributes(new { style = "text-align:center;" });
        }

        public static GridTemplateColumnBuilder<T> Lefted<T>(this GridTemplateColumnBuilder<T> columnBuilder) where T : class
        {
            return columnBuilder.HtmlAttributes(new { align = "left" })
                            .HeaderHtmlAttributes(new { style = "text-align:center;" });
        }
		...
		public static SelectList ToEnumSelectList<TEnum>(this TEnum enumObj, Func<TEnum, bool> predicate = null, bool addPleaseSelect = false)
        {
            ...
            var outputs = (from TEnum e in values
                           select new SelectListItem { Value = e.ToString(), Text = (e as Enum).ToString() });

            if (addPleaseSelect)
            {
                var pleaseSelect = new List<SelectListItem> { new SelectListItem { Text = "", Value = "" } };
        ...
        public static SelectList ToEnumSelectList<TEnum>(this TEnum enumObj, Func<TEnum, bool> predicate = null, string defaultText = "")
        {
            IEnumerable<TEnum> values = (from TEnum e in Enum.GetValues(typeof(TEnum))
                                         select e);
            ...
            var outputs = (from TEnum e in values
                           select new SelectListItem { Value = Convert.ToInt32(e).ToString(), Text = (e as Enum).ToString() });

            if (!string.IsNullOrEmpty(defaultText))
            {
                var pleaseSelect = new List<SelectListItem> { new SelectListItem { Text = defaultText, Value = "0" } };
                outputs = pleaseSelect.Concat(outputs).ToList();
            }
        ...
        public static SelectList ToEnumSelectList<TEnum>(this TEnum enumObj, Func<TEnum, bool> predicate = null, bool addPleaseSelect = false, string selected = "")
        {
            IEnumerable<TEnum> values = (from TEnum e in Enum.GetValues(typeof(TEnum))
                                         select e);
            ...
            var outputs = (from TEnum e in values
                           select new SelectListItem { Value = e.ToString(), Text = (e as Enum).GetDescriptionString(), Selected = (selected.Contains(e.ToString())) });

            if (addPleaseSelect)
            {
                var pleaseSelect = new List<SelectListItem> { new SelectListItem { Text = "", Value = "", Selected = (selected == "") } };
                outputs = pleaseSelect.Concat(outputs).ToList();
            }

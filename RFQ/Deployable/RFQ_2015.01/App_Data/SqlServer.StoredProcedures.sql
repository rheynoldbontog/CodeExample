CREATE FUNCTION [dbo].[ssg_splitstring_to_table]
(
    @string NVARCHAR(MAX),
    @delimiter CHAR(1)
)
...
CREATE FUNCTION [dbo].[ssg_getnotnullnotempty]
(
...
CREATE FUNCTION [dbo].[ssg_getprimarykey_indexname]
(
...
BEGIN
	...
	SELECT CAST(data as int) FROM [ssg_splitstring_to_table](@CategoryIds, ',')	
	...
	SELECT CAST(data as int) FROM [ssg_splitstring_to_table](@FilteredSpecs, ',')	
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SSGTemplateFullTextCatalog'')
		CREATE FULLTEXT CATALOG [SSGTemplateFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[ssg_getprimarykey_indexname] ('Product') +  '] ON [SSGTemplateFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductVariant]''))
		...
		KEY INDEX [' + dbo.[ssg_getprimarykey_indexname] ('ProductVariant') +  '] ON [SSGTemplateFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[ssg_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [SSGTemplateFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[ssg_getprimarykey_indexname] ('ProductTag') +  '] ON [SSGTemplateFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''SSGTemplateFullTextCatalog'')
		DROP FULLTEXT CATALOG [SSGTemplateFullTextCatalog]
	')
END
GO

    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetSiteHost(bool useSsl)
        {
            ...
            else
            {
                ...
                //so let's resolve SiteInformationSettings here.
            ...
            else
            {
                if (SslEnabled())
                {
                    ...
                    if (!String.IsNullOrEmpty(sharedSslUrl))
                    {
                        ...
                        /* we need to set a site URL here (IoC.Resolve<ISettingManager>().SiteUrl property)
                         * but we cannot reference SSG.BusinessLogic.dll assembly.
                         * So we are using one more app config settings - <add key="NonSharedSSLUrl" value="http://www.yourSite.com" />
                         */
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false, string redirectUrl = "")
        {
            ...
            else
            {
                ...
                if (!success)
                {
                    throw new SSGException("SSGTemplate needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        private bool TryWriteGlobalAsax()
        {
            try
            {
                //When a new plugin is dropped in the Plugins folder and is installed into SSGTemplate, 
                ...
                //The issue is described here: http://www.SSGTemplate.com/boards/t/10969/ssg-20-plugin.aspx?p=4#51318
        ...
        public virtual bool IsSearchEngine(HttpContextBase context)
        {
            ...
            //more info: http://www.SSGTemplate.com/boards/t/17711/unhandled-exception-request-is-not-available-in-this-context.aspx
            if (context == null)
                return false;
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContext.Items["ssg.IsPOSTBeingDone"] == null)
                    return false;
                return Convert.ToBoolean(_httpContext.Items["ssg.IsPOSTBeingDone"]);
            }
            set
            {
                _httpContext.Items["ssg.IsPOSTBeingDone"] = value;
            }
        }
    }
}

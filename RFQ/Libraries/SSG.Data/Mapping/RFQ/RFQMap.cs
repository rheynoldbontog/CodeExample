...
namespace SSG.Data.Mapping.RFQ
{
    public class RFQMap : EntityTypeConfiguration<SSG.Core.Domain.RFQ.RFQ>
    {
        public RFQMap()
        {
            ...
            this.Property(t => t.SubmittedDate).IsOptional().HasColumnName("SubmittedDate");
            this.Ignore(t => t.EmailAttachmentFilename);
            this.Ignore(t => t.EditUrl);
            this.Ignore(t => t.BuyerChangeReason);
            ...
            this.HasMany(x => x.BuyerChanges).WithRequired(x => x.RFQ).HasForeignKey(x => x.RFQId).WillCascadeOnDelete(false);

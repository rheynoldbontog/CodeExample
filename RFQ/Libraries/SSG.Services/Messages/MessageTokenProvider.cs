...
namespace SSG.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public void AddOpenRFQReportTokens(IList<Token> tokens)
        {
            
            tokens.Add(new Token("Report.CreatedDate", DateTime.Now.ToString("F")));
        }

        public void AddRFQLineTokens(IList<Token> tokens, SSG.Core.Domain.RFQ.RFQLine rfqLine)
        {
            tokens.Add(new Token("RFQLine.RFQLineNo", rfqLine.RFQLineNo.ToString()));
            tokens.Add(new Token("RFQLine.ItemDescription", rfqLine.ItemDescription));

            //event notification
            _eventPublisher.EntityTokensAdded(rfqLine, tokens);
        }

        public void AddRFQTokens(IList<Token> tokens, SSG.Core.Domain.RFQ.RFQ rfq)
        {
            tokens.Add(new Token("RFQ.RFQNo", rfq.RFQNo));
            tokens.Add(new Token("RFQ.Requester", rfq.Requestor.GetFullName()));
            tokens.Add(new Token("RFQ.Buyer", rfq.Buyer.GetFullName()));
            tokens.Add(new Token("RFQ.Requester.Department", rfq.Requestor.Department.Name));
            tokens.Add(new Token("RFQ.Details.Url", rfq.EditUrl));

            var linesTable = new StringBuilder();

            foreach (var line in rfq.Lines.Where(l => l.Active == true && l.RFQLineStatusId != (int)RFQLineStates.Deleted))
            {
                linesTable.AppendFormat(@"<tr>");
                linesTable.AppendFormat(@"<td style=""font-weight: normal; border: 1px solid #dddddd; text-align: center; padding: 5px;"">{0}</td>", line.RFQLineNo);
                linesTable.AppendFormat(@"<td style=""font-weight: normal; border: 1px solid #dddddd; text-align: center; padding: 5px;"">{0}</td>", line.ItemDescription);
                linesTable.AppendFormat(@"<td style=""font-weight: normal; border: 1px solid #dddddd; text-align: center; padding: 5px;"">{0}</td>", line.Quantity);
                linesTable.AppendFormat(@"<td style=""font-weight: normal; border: 1px solid #dddddd; text-align: center; padding: 5px;"">{0}</td>", line.Maker);
                linesTable.AppendFormat(@"<td style=""font-weight: normal; border: 1px solid #dddddd; text-align: center; padding: 5px;"">{0}</td>", line.MakerPN);
                linesTable.AppendFormat(@"</tr>");
            }

            tokens.Add(new Token("RFQ.Lines.Table", linesTable.ToString(), true));

            //event notification
            _eventPublisher.EntityTokensAdded(rfq, tokens);
        }

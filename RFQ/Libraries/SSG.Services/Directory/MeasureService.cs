...
namespace SSG.Services.Directory
{
    ...
    {
        #region Constants
        private const string MEASUREDIMENSIONS_ALL_KEY = "SSG.measuredimension.all";
        private const string MEASUREDIMENSIONS_BY_ID_KEY = "SSG.measuredimension.id-{0}";
        private const string MEASUREWEIGHTS_ALL_KEY = "SSG.measureweight.all";
        private const string MEASUREWEIGHTS_BY_ID_KEY = "SSG.measureweight.id-{0}";
        private const string MEASUREDIMENSIONS_PATTERN_KEY = "SSG.measuredimension.";
        private const string MEASUREWEIGHTS_PATTERN_KEY = "SSG.measureweight.";
        #endregion
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (result != decimal.Zero && sourceMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSGException(string.Format("Exchange ratio not set for dimension [{0}]", sourceMeasureDimension.Name));
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (result != decimal.Zero && targetMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSGException(string.Format("Exchange ratio not set for dimension [{0}]", targetMeasureDimension.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal quantity, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (result != decimal.Zero && sourceMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSGException(string.Format("Exchange ratio not set for weight [{0}]", sourceMeasureWeight.Name));
                result = result / exchangeRatio;
            }
            return result;
        }
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (result != decimal.Zero && targetMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new SSGException(string.Format("Exchange ratio not set for weight [{0}]", targetMeasureWeight.Name));
                result = result * exchangeRatio;
            }
            return result;
        }

...
namespace SSG.Services.RFQ
{
    ...
    {
        public static void SetEmailAttachmentFilePath(this SSG.Core.Domain.RFQ.RFQ rfq, string dirPath)
        {
            rfq.EmailAttachmentFilename = Path.Combine(dirPath, String.Format("{0}_{1}.pdf", rfq.RFQNo, DateTime.Now.ToString("yyyyMMddTHHmmss")));
            //rfq.EmailAttachmentFilename = Path.Combine(dirPath, String.Format("add3Dgraph1.pdf", rfq.RFQNo));
        }
        ...
        public static bool UserCanSubmit(this SSG.Core.Domain.RFQ.RFQ rfq, User currentUser)
        {
            ...
            try
            {
                if (rfq.RFQStatus.Name.Equals(Enum.GetName(typeof(RFQStates), RFQStates.Draft)) && rfq.RequestorId == currentUser.Id && rfq.Lines.Count > 0)
                    result = true;
                else
                    result = false;
            }

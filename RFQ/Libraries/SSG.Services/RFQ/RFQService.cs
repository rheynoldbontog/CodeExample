    ...
    public class RFQService : IRFQService
    {
        ...
        { 
            ...
            if (rfq.Id > 0)
            {
                ...
                var buyerChangeReason = rfq.BuyerChangeReason;
                                
                _rfqRepository.Update(rfq);

                //Notify status change if applicable...
                NotifyStatusChanged(oldStatusId, rfq);
                NotifyBuyerChanged(oldBuyerId, rfq, buyerChangeReason);
            }
            else
            {
                _rfqRepository.Insert(rfq);
        ...
        private void NotifyBuyerChanged(int oldBuyerId, SSG.Core.Domain.RFQ.RFQ updatedRFQ, string changeReason)
        {
            if (oldBuyerId != updatedRFQ.BuyerId && updatedRFQ.RFQStatusId == (int)RFQStates.Submitted)
            {
                ...
                if (changeReason == null)
                    changeReason = "Buyer Changed From ID: " + oldBuyerId.ToString() + " to ID: " + updatedRFQ.BuyerId.ToString();

                _rfqBuyerHistoryService.SaveBuyerHistory(updatedRFQ.Id, oldBuyerId, updatedRFQ.BuyerId, changeReason, _workContext.CurrentUser.Id);
            }
        }

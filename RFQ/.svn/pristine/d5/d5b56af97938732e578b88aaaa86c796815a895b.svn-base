...
namespace SSG.Services.RFQ
{
    public class ReportService : IReportService
    {
        ...
        public void createExcel(string filePath, List<RFQLine> rfqLines)
        {
            ...
            using (var xlPackage = new ExcelPackage(newFile))
            {
                ...
                foreach (var rfqLine in rfqLines)
                {
                    ...
                    worksheet.Cells[row, col].Value = rfqLine.LineType.Name;
                    worksheet.Column(col).AutoFit();
                    col++;
                    ...
                    worksheet.Cells[row, col].Value = DateTime.Today.AddDays(rfqLine.DateCreatedOnUtc.Day).Day;
                    worksheet.Column(col).AutoFit();
                    col++;
        ...
        public void ExportAllRFQIndex(string filePath, List<RFQLineService.TableJoin> rfqTableJoins)
        {
            var newFile = new FileInfo(filePath);
            using (var xlPackage = new ExcelPackage(newFile))
            {
                var worksheet = xlPackage.Workbook.Worksheets.Add("RFQ");
                var properties = new string[]
                {
                    "RFQ",
                    "Subject",
                ...
                int row = 2;
                foreach (var rfqall in rfqTableJoins)
                {
                    int col = 1;

                    worksheet.Cells[row, col].Value = rfqall.Rfq.RFQNo;
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.Rfq.Subject;
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.Rfq.RFQStatus.Name;
                    worksheet.Column(col).AutoFit();
                    col++;
                    if (rfqall.RfqLine != null)
                    {
                        worksheet.Cells[row, col].Value = rfqall.RfqLine.RFQLineNo;
                        worksheet.Column(col).AutoFit();
                        col++;
                    }
                    else
                    ...
                    worksheet.Cells[row, col].Value = rfqall.Rfq.DateCreatedOnUtc.ToString("d");
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.ItemDescription : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    if (rfqall.RfqLine != null)
                    {
                        worksheet.Cells[row, col].Value = rfqall.RfqLine.Quantity;
                        worksheet.Column(col).AutoFit();
                        col++;
                    }
                    else
                    ...
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.QuantityUnit.Name : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.MakerPN : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.Rfq.Requestor.GetFullName() : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.Rfq.Department : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.Rfq.Buyer.GetFullName() : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.Status.Name : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.FormType.Name : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    if (rfqall.RfqLine != null)
                    {
                        if (rfqall.RfqLine.Quotations.Count > 0)
                        {
                            var quotations = new StringBuilder();
                            int i = 1;
                            foreach (var quote in rfqall.RfqLine.Quotations)
                            {
                                quotations.Append(quote.Commodity);
                                if (i != rfqall.RfqLine.Quotations.Count)
                                {
                                    quotations.Append(",");
                                }
                                i++;
                            }
                            worksheet.Cells[row, col].Value = quotations;
                            worksheet.Column(col).AutoFit();
                            col++;
                        }
                        else
                    ...
                    if (rfqall.RfqLine != null)
                    {
                        worksheet.Cells[row, col].Value = rfqall.RfqLine.ROHSCompliant;
                        worksheet.Column(col).AutoFit();
                        col++;
                    }
                    else
                    ...
                    worksheet.Cells[row, col].Value = (rfqall.RfqLine != null ? (rfqall.RfqLine.FormType.Id == 2 ? rfqall.RfqLine.TestEquipmentPurchaseType.Name : null) : null);
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = (rfqall.RfqLine != null ? (rfqall.RfqLine.FormType.Id == 2 ? rfqall.RfqLine.TestEquipmentCalibrationType.Name : null) : null);
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = rfqall.RfqLine != null ? rfqall.RfqLine.NoteToBuyer : "";
                    worksheet.Column(col).AutoFit();
                    col++;
                    worksheet.Cells[row, col].Value = (rfqall.RfqLine != null ? (rfqall.RfqLine.FirstUploadDate != null ? Convert.ToDateTime(rfqall.RfqLine.FirstUploadDate).ToString("d") : null) : null);
                    worksheet.Column(col).AutoFit();
                    col++;

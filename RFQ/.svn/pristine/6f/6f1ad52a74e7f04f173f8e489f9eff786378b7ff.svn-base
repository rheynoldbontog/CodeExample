...
namespace SSG.Web.Infrastructure.Installation
{
    /// <summary>
    ...
    {
        private const string LanguageCookieName = "ssg.installation.lang";
                ...
                foreach (var filePath in Directory.EnumerateFiles(webHelper.MapPath("~/App_Data/Localization/Installation/"), "*.xml"))
                {
                    ...
                    foreach (XmlNode resNode in xmlDocument.SelectNodes(@"//Language/LocaleResource"))
                    {
                        ...
                        if (resNameAttribute == null)
                            throw new SSGException("All installation resources must have an attribute Name=\"Value\".");
                        var resourceName = resNameAttribute.Value.Trim();
                        if (string.IsNullOrEmpty(resourceName))
                            throw new SSGException("All installation resource attributes 'Name' must have a value.'");

                        if (resValueNode == null)
                            throw new SSGException("All installation resources must have an element \"Value\".");
